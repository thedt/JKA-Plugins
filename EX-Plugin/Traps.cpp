...
{
	return (qboolean)syscall(G_ICARUS_REGISTERSCRIPT, name, bCalledDuringInterrogate);
}
...
{
	return (qboolean)syscall(G_ICARUS_VALIDENT, ent);
}

qboolean trap_ICARUS_IsInitialized( int entID )
{
	return (qboolean)syscall(G_ICARUS_ISINITIALIZED, entID);
}

qboolean trap_ICARUS_MaintainTaskManager( int entID )
{
	return (qboolean)syscall(G_ICARUS_MAINTAINTASKMANAGER, entID);
}

qboolean trap_ICARUS_IsRunning( int entID )
{
	return (qboolean)syscall(G_ICARUS_ISRUNNING, entID);
}

qboolean trap_ICARUS_TaskIDPending(gentity_t *ent, int taskID)
{
	return (qboolean)syscall(G_ICARUS_TASKIDPENDING, ent, taskID);
}
...
{
	return (qboolean)syscall(G_NAV_LOAD, filename, checksum);
}

qboolean trap_Nav_Save( const char *filename, int checksum )
{
	return (qboolean)syscall(G_NAV_SAVE, filename, checksum);
}
...
{
	return (qboolean)syscall(G_NAV_CONNECTED, startID, endID);
}
...
{
	return (qboolean)syscall(G_NAV_NODEFAILED, ent, nodeID);
}

qboolean trap_Nav_NodesAreNeighbors( int startID, int endID )
{
	return (qboolean)syscall(G_NAV_NODESARENEIGHBORS, startID, endID);
}
...
{
	return (qboolean)syscall(G_NAV_CHECKFAILEDEDGE, failedEdge);
}
...
{
	return (qboolean)syscall(G_NAV_ROUTEBLOCKED, startID, testEdgeID, endID, rejectRank);
}
...
{
	return (qboolean)syscall(G_NAV_GETPATHSCALCULATED);
}
...
{
	return (qboolean)syscall(G_G2_SETSKIN, ghoul2, modelIndex, customSkin, renderSkin);
}
...
{
	return (qboolean)syscall(G_G2_GETBONEANIM, ghoul2, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList, modelIndex);
}
...
{
	return (qboolean)syscall(G_G2_REMOVEGHOUL2MODELS, ghlInfo);
}
...
{
	return (qboolean)syscall(G_G2_SETROOTSURFACE, ghoul2, modelIndex, surfaceName);
}

qboolean trap_G2API_SetSurfaceOnOff(void *ghoul2, const char *surfaceName, const int flags)
{
	return (qboolean)syscall(G_G2_SETSURFACEONOFF, ghoul2, surfaceName, flags);
}

qboolean trap_G2API_SetNewOrigin(void *ghoul2, const int boltIndex)
{
	return (qboolean)syscall(G_G2_SETNEWORIGIN, ghoul2, boltIndex);
}
...
{
	return (qboolean)syscall(G_G2_DOESBONEEXIST, ghoul2, modelIndex, boneName);
}
...
{
	return (qboolean)syscall(G_G2_RAGPCJCONSTRAINT, ghoul2, boneName, min, max);
}
...
{
	return (qboolean)syscall(G_G2_RAGPCJGRADIENTSPEED, ghoul2, boneName, PASSFLOAT(speed));
}
...
{
	return (qboolean)syscall(G_G2_RAGEFFECTORGOAL, ghoul2, boneName, pos);
}
...
{
	return (qboolean)syscall(G_G2_GETRAGBONEPOS, ghoul2, boneName, pos, entAngles, entPos, entScale);
}

qboolean trap_G2API_RagEffectorKick(void *ghoul2, const char *boneName, vec3_t velocity) //add velocity to a rag bone
{
	return (qboolean)syscall(G_G2_RAGEFFECTORKICK, ghoul2, boneName, velocity);
}
...
{
	return (qboolean)syscall(G_G2_RAGFORCESOLVE, ghoul2, force);
}
...
{
	return (qboolean)syscall(G_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState, params);
}

qboolean trap_G2API_IKMove(void *ghoul2, int time, sharedIKMoveParams_t *params)
{
	return (qboolean)syscall(G_G2_IKMOVE, ghoul2, time, params);
}

qboolean trap_G2API_RemoveBone(void *ghoul2, const char *boneName, int modelIndex)
{
	return (qboolean)syscall(G_G2_REMOVEBONE, ghoul2, boneName, modelIndex);
}
...
{
	return (qboolean)syscall(G_G2_OVERRIDESERVER, serverInstance);
}
