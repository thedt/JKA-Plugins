...
template <typename T>
	...
	public:
		...
		std::string Encrypt()
		{
			...
			{
				_error = true;
				errormsg = JASS_WRITEJASSLOG("You need to set key and\or plain text");
			}
			if(enc == false) //We haven't already crypted our string..so we need to encrypt it!
			{
				...
				CryptoPP::StringSource( _plain, true,new CryptoPP::StreamTransformationFilter( _enc, new CryptoPP::StringSink( _encrypted ))); //Crypting and converting to string...
					...
					/*std::string errText =*/ JASS_WRITEJASSLOG("Bad encrypt");
				errormsg = e.GetWhat();
				_error = true;
				return errormsg; 
				}	
		...
		void setKey(std::string key)
		{
			...
			{
				_error = true;
				errormsg= JASS_WRITEJASSLOG("No valid Key Length");
			}
		...
		void setIV(std::string iv)
		{
			...
			{
				_error = true;
				errormsg= JASS_WRITEJASSLOG("No valid IV Length");
			}
			if(_error == false)
			{
				ivsetted=true;
		...
		void setPlainString(std::string plainstring)
		{
			...
			{
				_error = true;
				errormsg= JASS_WRITEJASSLOG("No valid Plain String Length");
			}
			if(_error == false)
				_plain = plainstring;
		}
		...
		{	
				...
				{
					_error = true;
					errormsg= JASS_WRITEJASSLOG("No valid Encrypted String Length");
				}
				if(_error == false)
	...
	public:
		...
		std::string Encrypt()
		{
			...
			{
				_error = true;
				errormsg = JASS_WRITEJASSLOG("You need to set plain text");
				return errormsg;
			}
			...
			if(_error == false)
			{
				...
				if(_encrypted != "")
				{
					...
					std::string errText = JASS_WRITEJASSLOG("Bad Hash encrypt");
				errormsg = e.GetWhat();
				_error = true;
				return errText; 
				}	
		...
		void setPlain(std::string plain)
		{
			...
			{
				_error = true;
				errormsg = JASS_WRITEJASSLOG("No plain text specified");
			}
			if(_plain = plain && _error != true)
				_error = false;
			else
				errormsg = JASS_WRITEJASSLOG("Error during plain text assign");
