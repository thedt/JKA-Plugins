...
char * GetArgument(int num)
{
	char arg[256];
	trap_Argv(num,arg,sizeof(arg));
	return arg;
}
	...
	for (i = 0, cl = level.clients ; i < level.maxclients ; i++,cl++ ) {
		...
		if ( strstr( n2, s2 ) ) {
			if(idnum == -1)
				...
				trap_SendServerCommand( to-g_entities, va("print \"^1Too many users match this name\n\"", s));
				return -1;
			}
		}
	}
...
int ClientNumFromString(gentity_t *to, char *s)
{
	gentity_t	*ent;
	int			idnum, i, returned;
	char		s2[MAX_STRING_CHARS];
	char		n2[MAX_STRING_CHARS];
	char s3[MAX_STRING_CHARS];
	char n3[MAX_STRING_CHARS];
	idnum = atoi ( s );
	
	if ((s[0] == '-' && s[1] == '1') || s[0] == '0' || s[0] == '1' || s[0] == '2' || s[0] == '3' || s[0] == '4' || s[0] == '5' || s[0] == '6' || s[0] == '7' || s[0] == '8' || s[0] == '9') 
	return idnum;
	else
	{
		...
		for (i=0;i<level.maxclients;i++)
		{
			...
			SanitizeString(n2,n3);
			SanitizeString(s2,s3);
			if (strstr(n3,s3))
			{
				returned = 1;
				return i;
		...
		if (returned != 1)
		{
			trap_SendServerCommand(to-g_entities,va("print \"^1Name string ^7%s ^1not found on the server, assuming client number 0\n\"",s));
			return 0;
		}
	}
}

gentity_t* GEntityFromString( gentity_t *ent, char *s )
{
	int clientNum;
	gentity_t *target;
	clientNum = ClientNumFromString(ent,s);
	target = &g_entities[clientNum];
	return target;
}

void dtChat( int clientNum, char * str )
{
	return trap_SendServerCommand(clientNum, va("chat \"%s\"",str));
}

void dtConsole( int clientNum, char * str )
{
	return trap_SendServerCommand(clientNum, va("print \"%s\"",str));
}

void dtScreen( int clientNum, char * str )
{
	return trap_SendServerCommand(clientNum, va("cp \"%s\"",str));
}
...
Prints text on any single individual's screen
==================
*/

void Cmd_showtext_f (gentity_t *ent)
{
int num;
int formatnum;
gentity_t *formatc;
char target[MAX_STRING_CHARS];
char mode[MAX_STRING_CHARS];
char msg[MAX_STRING_CHARS];
char format[MAX_STRING_CHARS];
char health[MAX_STRING_CHARS];
char shield[MAX_STRING_CHARS];
// Admin check
/*
     if (IsCmdAllowed("showtext",ent->client->pers.id,ent) == 0)
     {
          trap_SendServerCommand(ent-g_entities,va("print \"^3[^5You are not logged in as an admin. Please do /adminlogin <password>.^3]\n\""));
		  return;
     }
	 */
if (trap_Argc() != 4 && trap_Argc() != 5)
{
	trap_SendServerCommand(ent-g_entities,"print \"^3[^5Command format: /showtext <clientnumber/name> <mode: screen/chat/corner> <message> <format client number/partial name - optional>^3]\n\"");
     return;
}
trap_Argv(2,mode,sizeof(mode));
trap_Argv(1,target,sizeof(target));
num = ClientNumFromString(ent,target);
trap_Argv(3,msg,sizeof(msg));
if (trap_Argc() == 5)
{
	trap_Argv(4,format,sizeof(format));
	formatnum = ClientNumFromString(ent,format);
	formatc = &g_entities[formatnum];
	itoa(formatc->client->ps.stats[STAT_HEALTH],health,10);
	itoa(formatc->client->ps.stats[STAT_ARMOR],shield,10);
	strncpy(msg,str_replace(msg,"$n",formatc->client->pers.netname),sizeof(msg));
	strncpy(msg,str_replace(msg,"$h",health),sizeof(msg));
	strncpy(msg,str_replace(msg,"$s",shield),sizeof(msg));
}
		   if (strcmp(mode, "screen") == 0)
			trap_SendServerCommand(num,va("cp \"%s\"",msg));
		   else if (strcmp(mode, "chat") == 0)
          trap_SendServerCommand(num,va("chat \"^5Announcement: ^3[ ^5%s ^3]\"",msg));
		   else if (strcmp(mode, "corner") == 0)
			   trap_SendServerCommand(num,va("print \"^5Announcement: ^3[ ^5%s ^3]\n\"",msg));
		   else
			   trap_SendServerCommand(ent-g_entities,va("print \"^5You did not specify a valid mode.\n\""));
}
...
void ClientCommand( int clientNum ) {
	...
	if (level.intermissiontime)
	{
		...
		else if (!Q_stricmp(cmd, "vipnoclip"))
		{
			giveError = qtrue;
		}
	...
	else if (Q_stricmp (cmd, "vipgod") == 0)
		Cmd_God_f (ent);
	else if (Q_stricmp (cmd, "notarget") == 0)
		Cmd_Notarget_f (ent);
	else if (Q_stricmp (cmd, "vipnoclip") == 0)
		Cmd_Noclip_f (ent);
	else if ( Q_stricmp( cmd, "NPC" ) == 0 && CheatsOk(ent) )
	{
		...
		Cmd_Stats_f( ent );
	else if (Q_stricmp (cmd, "showtext") == 0)
		Cmd_showtext_f( ent );
	/*
